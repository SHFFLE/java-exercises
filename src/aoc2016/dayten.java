...
public class dayten {
    public static List<Bot> ListOfBots = new ArrayList<>();

    public static void main(String[] args) {
        String[] input = {"bot 127 gives low to output 1 and high to bot 180",
...
//            int indexOfBot = current.indexOf("bot");
//            int bot1;
//            int value;
//            if (indexOfBot == 16 || indexOfBot == 17) {
//                value = Integer.valueOf(current.substring(current.indexOf("value") + 6, current.indexOf("goes") - 1));
//                bot1 = Integer.valueOf(current.substring(indexOfBot + 4, current.length()));
//                int bot1Index = findIndexOfBot(bot1);
//                if (ListOfBots.get(bot1Index).a == value || ListOfBots.get(bot1Index).b == value) {
//                } else if (ListOfBots.get(bot1Index).a == 0) {
//                    ListOfBots.get(bot1Index).a = value;
//                } else {
//                    ListOfBots.get(bot1Index).b = value;
//                }
//            } else {
//                String currentsub1 = current.substring(indexOfBot + 4, current.length());
//                bot1 = Integer.valueOf(currentsub1.substring(0, currentsub1.indexOf("gives") - 1));
//                int bot1Index = findIndexOfBot(bot1);
//                if (currentsub1.indexOf("bot") > 0 && (currentsub1.indexOf("bot") < currentsub1.indexOf("output") || currentsub1.indexOf("output") == -1)) {
//                    String currentsub2 = currentsub1.substring(currentsub1.indexOf("bot") + 4, currentsub1.length());
//                    int bot2 = Integer.valueOf(currentsub2.substring(0, currentsub2.indexOf("and") - 1));
//                    int bot2Index = findIndexOfBot(bot2);
//                    value = ListOfBots.get(bot1Index).findLow();
//                    if (ListOfBots.get(bot2Index).a == 0) {
//                        ListOfBots.get(bot2Index).a = value;
//                    } else {
//                        ListOfBots.get(bot2Index).b = value;
//                    }
//                    if (currentsub2.contains("bot")) {
//                        String currentsub3 = currentsub2.substring(currentsub2.indexOf("bot") + 4, currentsub2.length());
//                        int bot3 = Integer.valueOf(currentsub3);
//                        int bot3Index = findIndexOfBot(bot3);
//                        value = ListOfBots.get(bot1Index).findHigh();
//                        if (ListOfBots.get(bot3Index).a == 0) {
//                            ListOfBots.get(bot3Index).a = value;
//                        } else {
//                            ListOfBots.get(bot3Index).b = value;
//                        }
//                    }
//                } else {
//                    String currentsub2 = currentsub1.substring(currentsub1.indexOf("output") + 6, currentsub1.length());
//                    if (currentsub2.contains("bot")) {
//                        String currentsub3 = currentsub2.substring(currentsub2.indexOf("bot") + 4, currentsub2.length());
//                        int bot3 = Integer.valueOf(currentsub3);
//                        int bot3Index = findIndexOfBot(bot3);
...
//        }
        for (int i = 0; i < input.length; i++) {
            ...
            if (indexOfBot == 16 || indexOfBot == 17) {
                ...
                bot1 = Integer.valueOf(current.substring(indexOfBot + 4, current.length()));
                int bot1Index = findIndexOfBot(bot1);
                ListOfBots.get(bot1Index).add(value);
            } else {
                String currentsub1 = current.substring(indexOfBot + 4, current.length());
                bot1 = Integer.valueOf(currentsub1.substring(0, currentsub1.indexOf("gives") - 1));
                int bot1Index = findIndexOfBot(bot1);
                if (currentsub1.indexOf("bot") > 0 && (currentsub1.indexOf("bot") < currentsub1.indexOf("output") || currentsub1.indexOf("output") == -1)) {
                    ...
                    int bot2Index = findIndexOfBot(bot2);
                    value = ListOfBots.get(bot1Index).findLow();
                    ListOfBots.get(bot2Index).add(value);
                    if (currentsub2.contains("bot")) {
                        String currentsub3 = currentsub2.substring(currentsub2.indexOf("bot") + 4, currentsub2.length());
                        int bot3 = Integer.valueOf(currentsub3);
                        int bot3Index = findIndexOfBot(bot3);
                        value = ListOfBots.get(bot1Index).findHigh();
                        ListOfBots.get(bot3Index).add(value);
                    }
                } else {
                    String currentsub2 = currentsub1.substring(currentsub1.indexOf("output") + 6, currentsub1.length());
                    ListOfBots.get(bot1Index).findLow();
                    if (currentsub2.contains("bot")) {
                        String currentsub3 = currentsub2.substring(currentsub2.indexOf("bot") + 4, currentsub2.length());
                        int bot3 = Integer.valueOf(currentsub3);
                        int bot3Index = findIndexOfBot(bot3);
    ...
    public static void checkAllForAnswers(){
        for (int i = 0; i < ListOfBots.size(); i++) {
            System.out.println("ID: " + ListOfBots.get(i).id);
            for (int j = 0; j < ListOfBots.get(i).values.size(); j++) {
                System.out.println(ListOfBots.get(i).values.get(j));
            }
        }
    }

    public static void checkForAnswer(int botIndex){

        if(ListOfBots.get(botIndex).a == 61 && ListOfBots.get(botIndex).b == 17 || ListOfBots.get(botIndex).a == 17 && ListOfBots.get(botIndex).b == 61){
            System.out.println(ListOfBots.get(botIndex).id + " is the responsible bot");
        }
    }

    public static void createIfNotExists(int botID){
        boolean exists = false;
        for (int i = 0; i < ListOfBots.size(); i++) {
            if(ListOfBots.get(i).id == botID){
                exists = true;
            }
        }
        if(!exists){
            ListOfBots.add(new Bot(botID));
        }
    }
    public static int findIndexOfBot(int botID){
        for (int i = 0; i < ListOfBots.size(); i++) {
            if(ListOfBots.get(i).id == botID){
                return i;
            }
        }
        createIfNotExists(botID);
        return findIndexOfBot(botID);
    }
}
