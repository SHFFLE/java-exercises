...
public class dayseven {
    public static void main(String[] args) {
        String[] input = {"dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh" ,
        ...
        for (int i = 0; i < input.length; i++) {
            List<String> listOfABAs = new ArrayList<>();
            String pt1 = input[i].substring(0, input[i].indexOf('['));
            String brackets = input[i].substring(input[i].indexOf('[')+1, input[i].indexOf(']'));
            String pt2 = input[i].substring(input[i].indexOf(']')+1, input[i].length());
            String brackets2 = "";
            String pt3 = "";
            String brackets3 = "";
            String pt4 = "";
            ...
            String pt5 = "";
            if(pt2.indexOf('[') > 0){
                brackets2 = pt2.substring(pt2.indexOf('[')+1, pt2.indexOf(']'));
                pt3 = pt2.substring(pt2.indexOf(']')+1, pt2.length());
                pt2 = pt2.substring(0, pt2.indexOf('['));
            }
            if(pt3.indexOf('[') > 0){
                brackets3 = pt3.substring(pt3.indexOf('[')+1, pt3.indexOf(']'));
                pt4 = pt3.substring(pt3.indexOf(']')+1, pt3.length());
                pt3 = pt3.substring(0, pt3.indexOf('['));
            }
            if(pt4.indexOf('[') > 0){
                brackets4 = pt4.substring(pt4.indexOf('[')+1, pt4.indexOf(']'));
                pt5 = pt4.substring(pt4.indexOf(']')+1, pt4.length());
                pt4 = pt4.substring(0, pt4.indexOf('['));
            }
            boolean BAB = false;
            boolean isValid = false;
            if(checkOutOfBrackets(pt1)){
                isValid = true;
            }
            checkABA(pt1, listOfABAs);
            if(checkOutOfBrackets(pt2)){
                isValid = true;
            }
            checkABA(pt2, listOfABAs);
            if(checkOutOfBrackets(pt3)){
                isValid = true;
            }
            checkABA(pt3, listOfABAs);
            if(checkOutOfBrackets(pt4)){
                isValid = true;
            }
            checkABA(pt4, listOfABAs);
            if(checkOutOfBrackets(pt5)){
                isValid = true;
            }
            checkABA(pt5, listOfABAs);

            if(!checkBrackets(brackets)){
                isValid = false;
            }
            if(checkBAB(brackets, listOfABAs)){
                BAB = true;
            }

            if(!checkBrackets(brackets2)){
                isValid = false;
            }
            if(checkBAB(brackets2, listOfABAs)){
                BAB = true;
            }

            if(!checkBrackets(brackets3)){
                isValid = false;
            }
            if(checkBAB(brackets3, listOfABAs)){
                BAB = true;
            }

            if(!checkBrackets(brackets4)){
                isValid = false;
            }
            if(checkBAB(brackets4, listOfABAs)){
                BAB = true;
            }
        ...
        System.out.println(BABcount);
    }

    public static boolean checkOutOfBrackets(String input){
        boolean toReturn = false;
        for (int j = 0; j < input.length()-3; j++) {
            char a = input.charAt(j);
            char b = input.charAt(j+1);
            if(a == input.charAt(j+3) && b == input.charAt(j+2) && a != b){
                toReturn = true;
            }
        }
        return toReturn;
    ...
    public static List<String> checkABA(String input, List<String> list){
        for (int j = 0; j < input.length()-2; j++) {
            char a = input.charAt(j);
            char b = input.charAt(j+1);
            if(a == input.charAt(j+2) && a != b) {
                String aba = "";
                aba += a;
                aba += b;
                aba += a;
    ...
    public static boolean checkBAB(String input, List<String> list){
        boolean toReturn = false;
        for (int i = 0; i < list.size(); i++) {
            String ABA = list.get(i);
            String BAB = "";
            BAB += ABA.charAt(1);
            BAB += ABA.charAt(0);
            BAB += ABA.charAt(1);
            if(input.contains(BAB)){
                return true;
            }
        }
        return toReturn;
