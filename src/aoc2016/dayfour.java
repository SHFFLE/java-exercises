...
public class dayfour {
    public static void main(String[] args) {
        List<String> realRooms = new ArrayList<>();
        String[] input = {"aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]" ,
        ...
        for (int i = 0; i < input.length; i++) {
            ...
            charmap.put('y', 0);
            charmap.put('z', 0);
            for (int j = 0; j < input[i].substring(0, (input[i].length()-10)).length(); j++) {
                Integer a = charmap.get(input[i].charAt(j));
                if (input[i].charAt(j) != '-') {
                    charmap.put(input[i].charAt(j), a + 1);
                }
            }
            String properCode = "";
            char key = ' ';
            ...
            if((input[i].substring(input[i].length()-6, input[i].length()-1).equals(properCode))){
                int a = Integer.valueOf((input[i].substring(input[i].length()-10, input[i].length()-7)));
                realRooms.add(input[i]);
            }
        }
        for (int i = 0; i < realRooms.size(); i++) {
            String alphabet = "abcdefghijklmnopqrstuvwxyz";
            String current = realRooms.get(i);
            int currentID;
            realRooms.remove(i);
            currentID = Integer.valueOf(current.substring(current.length()-10, current.length()-7));
            current = current.substring(0, current.length()-10);
            String realCurrent = "";
            for (int j = 0; j < current.length(); j++) {
                if (current.charAt(j) == '-') {
                    realCurrent = realCurrent.concat(" ");
                } else {
                    char currentLetter = current.charAt(j);
                    int currentLetterAsInt = alphabet.indexOf(currentLetter);
                    int postTransformationInt = ((currentLetterAsInt + currentID) % 26);
                    char postTransformationChar = alphabet.charAt(postTransformationInt);
                    realCurrent += postTransformationChar;
                }
            }
            System.out.println(realCurrent + " " + currentID);
        }
    }
}
